...
object ActionCatalog {
    ...
    val actions: List<ActionEntry> = listOf(
        ...
        ActionEntry(
            id = "lock_screen",
            name = "Lock screen",
            description = "Turn the screen off via the accessibility service.",
            family = ActionFamily.UI_INTERACTION,
            keywords = listOf("lock", "off", "sleep", "power", "screen"),
            factory = { Action.LockScreen() },
        ),
        ActionEntry(
            id = "toast",
            name = "Toast",
            description = "Show a brief on-screen message.",
            family = ActionFamily.NOTIFICATIONS,
            keywords = listOf("toast", "popup", "message", "snackbar", "show"),
            factory = { Action.Toast(text = "") },
        ),
        ActionEntry(
            id = "open_url",
            name = "Open URL",
            description = "Open a URL in the default browser (or a specific app).",
            family = ActionFamily.APPS,
            keywords = listOf("open", "url", "browser", "link", "deeplink"),
            factory = { Action.OpenUrl(url = "") },
        ),
        ActionEntry(
            id = "copy_clipboard",
            ...
            keywords = listOf("copy", "clipboard", "paste", "system"),
            factory = { Action.CopyToClipboard(text = "") },
        ),
        ActionEntry(
            id = "get_clipboard",
            name = "Get clipboard",
            description = "Read the clipboard contents into a variable.",
            family = ActionFamily.DATA,
            keywords = listOf("clipboard", "get", "read", "paste"),
            factory = { Action.GetClipboard(intoVar = "") },
        ),
        ActionEntry(
            id = "vibrate",
            name = "Vibrate",
            description = "Trigger a one-shot device vibration.",
            family = ActionFamily.UI_INTERACTION,
            keywords = listOf("vibrate", "haptic", "buzz", "feedback"),
            factory = { Action.Vibrate() },
        ),
        ActionEntry(
            id = "tts",
            name = "Speak (TTS)",
            description = "Speak text aloud via the device's TextToSpeech engine.",
            family = ActionFamily.NOTIFICATIONS,
            keywords = listOf("tts", "speak", "say", "voice", "audio", "text to speech"),
            factory = { Action.Tts(text = "") },
        ),
        ActionEntry(
            id = "math",
            name = "Math",
            description = "Arithmetic between two values into a variable (+, −, ×, ÷, %, power).",
            family = ActionFamily.VARIABLES,
            keywords = listOf("math", "add", "subtract", "multiply", "divide", "arithmetic", "compute"),
            factory = { Action.Math(left = "0", op = com.flowdroid.common.flow.MathOp.ADD, right = "0", intoVar = "") },
        ),
        ActionEntry(
            id = "string_transform",
            name = "String transform",
            description = "uppercase / lowercase / trim / replace / substring / length / reverse.",
            family = ActionFamily.VARIABLES,
            keywords = listOf("string", "uppercase", "lowercase", "trim", "replace", "substring", "length", "transform"),
            factory = { Action.StringTransform(input = "", op = com.flowdroid.common.flow.StringOp.UPPER, intoVar = "") },
        ),
        ActionEntry(
            id = "date_format",
            name = "Date format",
            description = "Format a Unix-millis timestamp with a pattern (SimpleDateFormat).",
            family = ActionFamily.VARIABLES,
            keywords = listOf("date", "format", "time", "timestamp", "simpledateformat"),
            factory = { Action.DateFormat(intoVar = "now") },
        ),
        ActionEntry(
            id = "send_sms",
            name = "Send SMS",
            description = "Send a text message via SmsManager. Requires SEND_SMS permission (dangerous).",
            family = ActionFamily.NETWORK,
            keywords = listOf("sms", "text", "send", "message"),
            factory = { Action.SendSms(phoneNumber = "", body = "") },
        ),
        ActionEntry(
            id = "send_whatsapp",
            name = "Send WhatsApp",
            description = "Open WhatsApp with a pre-filled message (user taps Send).",
            family = ActionFamily.NETWORK,
            keywords = listOf("whatsapp", "wa", "send", "message", "chat"),
            factory = { Action.SendWhatsApp(phoneNumber = "", body = "") },
        ),
        ActionEntry(
            id = "send_telegram",
            name = "Send Telegram",
            description = "Open Telegram with a pre-filled message (user taps Send).",
            family = ActionFamily.NETWORK,
            keywords = listOf("telegram", "tg", "send", "message", "chat"),
            factory = { Action.SendTelegram(recipient = "", body = "") },
        ),
        ActionEntry(
            id = "send_email",
            name = "Send email",
            description = "Open the default email app with To/Subject/Body pre-filled.",
            family = ActionFamily.NETWORK,
            keywords = listOf("email", "mail", "send", "compose", "mailto"),
            factory = { Action.SendEmail(to = "") },
        ),
        ActionEntry(
            id = "mqtt_publish",
            name = "MQTT publish",
            description = "Publish a message to an MQTT topic (broker, topic, payload, QoS).",
            family = ActionFamily.NETWORK,
            keywords = listOf("mqtt", "iot", "broker", "publish", "topic", "payload", "mosquitto", "hivemq"),
            factory = { Action.MqttPublish(brokerUrl = "tcp://", topic = "") },
        ),
        // ── Timing ───────────────────────────────────────────────────────────
        ActionEntry(
            id = "delay",
